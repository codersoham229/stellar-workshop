  ...
  return (
    ...
    >
      ...
      <div className="fixed top-0 left-0 w-full h-0.5 bg-gradient-to-r from-border/20 via-border/40 to-border/20 z-50">
        <div 
          className="h-full bg-gradient-to-r from-accent via-purple-600 to-blue-600 will-change-transform shadow-sm"
          style={{ 
      ...
      <div className="hidden sm:flex fixed right-2 sm:right-4 lg:right-8 top-1/2 -translate-y-1/2 z-40">
            ...
            <div key={index} className="relative group">
              <div
                className={cn(
                  ...
                  "bg-background/95 backdrop-blur-md border border-border/60 shadow-xl z-50 transition-opacity duration-300",
                  activeSection === index ? "opacity-100" : "opacity-0 group-hover:opacity-100"
                )}
              >
                <div className="flex items-center gap-1 sm:gap-1.5 lg:gap-2">
                  <div className="w-1 sm:w-1.5 lg:w-2 h-1 sm:h-1.5 lg:h-2 rounded-full bg-accent animate-pulse" />
              ...
              <button
                ...
                className={cn(
                  ...
                  activeSection === index 
                    ? "bg-accent border-accent shadow-lg before:animate-ping before:bg-accent/20" 
                    : "bg-transparent border-muted-foreground/40 hover:border-accent/60 hover:bg-accent/10"
                )}
        ...
        <div className="absolute left-1/2 top-0 bottom-0 w-0.5 lg:w-px bg-gradient-to-b from-transparent via-accent/20 to-transparent -translate-x-1/2 -z-10" />
      </div>
      ...
      {sections.map((section, index) => (
        ...
        >
          <div className={cn(
            ...
            <h1 className={cn(
              ...
              {section.subtitle ? (
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <div className="text-accent text-[0.6em] sm:text-[0.7em] font-medium tracking-wider">
                    {section.subtitle}
                  </div>
                </div>
              ) : (
            ...
            <div className={cn(
              "text-foreground/70 leading-relaxed mb-8 sm:mb-10 text-base sm:text-lg lg:text-xl font-light",
              section.align === 'center' ? "max-w-full mx-auto text-center" : "max-w-full"
            )}>
              <p className="mb-3 sm:mb-4">{section.description}</p>
              {index === 0 && (
                <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm text-foreground/50 mt-4 sm:mt-6">
              ...
              <div className="grid gap-3 sm:gap-4 mb-8 sm:mb-10">
                {section.features.map((feature, featureIndex) => (
                  <div 
                    key={feature.title}
                    className={cn(
                      "group p-4 sm:p-5 lg:p-6 rounded-lg sm:rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur-md hover:bg-white/[0.06] transition-all duration-300 hover:shadow-lg hover:shadow-accent/5",
                      "hover:border-accent/30 hover:-translate-y-1"
                    )}
                  >
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="w-1.5 sm:w-2 h-1.5 sm:h-2 rounded-full bg-accent/60 mt-1.5 sm:mt-2 group-hover:bg-accent transition-colors flex-shrink-0" />
                ...
                {section.actions.map((action) => (
                  <button
                    ...
                    className={cn(
                      ...
                      "hover:shadow-lg focus:outline-none w-full sm:w-auto cursor-pointer",
                      action.variant === 'primary' 
                        ? "bg-accent text-white hover:bg-accent/90 shadow-lg shadow-accent/25 hover:shadow-accent/40" 
                        : "border border-white/15 bg-white/5 backdrop-blur-md hover:bg-white/10 hover:border-accent/40 text-foreground"
                    )}
                  >
  ...
  const demoSections = [
    {
      ...
      align: "left" as const,
      actions: [
        { label: "Reserve Tickets", variant: "primary" as const, onClick: () => {
          const el = document.getElementById("tickets-section");
          if (el) el.scrollIntoView({ behavior: "smooth" });
        } },
        { label: "View Agenda", variant: "secondary" as const, onClick: () => {
    ...
    },
    {
      ...
      align: "center" as const,
      actions: [
        { label: "Claim Pass Now", variant: "primary" as const, onClick: () => {
          const el = document.getElementById("tickets-section");
          if (el) el.scrollIntoView({ behavior: "smooth" });
        } }
      ]
